...
data_list_1 = {"Run name" : [],"Time" : [],"Place" : [],"Runners" : [],"Number of Survivors / Captures" : [],"WR Value per Runner" : [],"Score" : [],"Run link" : []}
data_list_2 = {"Player Name" : [],"Nationality" : [],"Total WR(s)" : [],"Survivor WR(s)" : [],"Beast WR(s)" : [],"WR Value" : [],"Total Score" : [],"Most value run" : []}
#ftf_run = {"name" : "name","runid" : "id","gametype": 0,"time" : 900,"placement" : 0,"total_run_in_category" : 0,"the_runners" : [],"random_count" : 0,"wr_val" : 0,"wr_value_per_person" : 0,"score" : 0,"link" : "link"}
for i in run_stats_datas:
    data_list_1["Run name"] += list(i["name"])
    data_list_1['Time'] += list(f"{i["time"]//60}" + ":" + f"{i["time"] - (i["time"]//60)*60}")
    data_list_1["Place"] += list(i["placement"])
    for j in i['the_runners']:
        p += f"{j}, "
    data_list_1["Runners"] += list(p)
    if i["gametype"] == 0:
        data_list_1["Number of Survivors / Captures"] += list((len(i["the_runners"]) + i['random_count']))
    else:
        data_list_1["Number of Survivors / Captures"] += list(i['random_count'])
    data_list_1['WR Value per Runner'] += list(i["wr_value_per_person"])
    data_list_1['Score'] += list(i["score"])
    data_list_1['Run link'] += list(i["link"])

#player_stats = {"name" : "name","country" : "country","total_wr" : 0,"survivor_wr" : 0,"beast_wr" : 0,"wr_value" : 0,"score" : 0,"most_value_run" : "run_name"}
for i in player_stats_datas:
    data_list_2['Player Name'] += list(i["name"])
    data_list_2['Nationality'] += list(i["country"])
    data_list_2['Total WR(s)'] += list(i["total_wr"])
    data_list_2['Survivor WR(s)'] += list(i["survivor_wr"])
    data_list_2['Beast WR(s)'] += list(i["beast_wr"])
    data_list_2['WR Value'] += list(i["wr_value"])
    data_list_2['Total Score'] += list(i["score"])
    data_list_2['Most value run'] += list(i["most_value_run"])
